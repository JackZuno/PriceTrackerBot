...
async def scheduled_message(context: ContextTypes.DEFAULT_TYPE):
    ...
    # Check if the bot is up and running and the notifications are on
    if not user_data or (user_data and user_data.get('bot_on') == False) or (user_data and user_data.get('notifications_on') == False):
    ...
    if items:
        ...
        try:
            await context.bot.send_message(chat_id=chat_id, text="No items found, you can disable the notifications with /stop")
            return ConversationHandler.END
        except Forbidden:
